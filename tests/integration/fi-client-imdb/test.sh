...
testIMDBBogusID(){
  ...
  result=$(fi-client-imdb -s "BOGUS") || exit=$?
...
testIMDBNonExistentID(){
  ...
  result=$(fi-client-imdb -s "tt0000000") || exit=$?
...
testIMDBData(){
  ...
  result=$(fi-client-imdb -s "tt0000001" | jq -r -c .) || exit=$?
...
testIMDBData2(){
  ...
  result=$(fi-client-imdb -s "tt0286486" | jq -r -c .) || exit=$?
  dc-tools::assert::equal "$exit" "0"
...
testIMDBImage(){
  ...
  fi-client-imdb -s --image=dump "tt0000001" > "$filename" || exit=$?
  dc-tools::assert::equal "$exit" "0"
