...
zul||zu|Zulu|zoulou
zun|||Zuni|zuni
zxx|||No linguistic content; Not applicable|pas de contenu linguistique; non applicable0
zza|||Zaza; Dimili; Dimli; Kirdki; Kirmanjki; Zazaki|zaza; dimili; dimli; kirdki; kirmanjki; zazaki"

  # XXX for some reason, people appear to use: https://www.ups.com/worldshiphelp/WS14/ENU/AppHelp/Codes/Language_Codes.htm
  local supplemental="ZHH|||Chinese (Cantonese)|
...
ESP|||Spanish|
TRK|||Turkish|"
  # Main 3 letters iso codes
  sed -E 's/^([a-z-]+)[|]([^|]*)[|]([^|]*)[|]([^|(;,]+).*/\1/' <<<"$list"
  # Alternative 3 letters iso codes
  sed -E 's/^([a-z-]+)[|]([^|]*)[|]([^|]*)[|]([^|(;,]+).*/\2/' <<<"$list" | grep -Ei "[a-z-]+"
  # 2 letters iso codes
  sed -E 's/^([a-z-]+)[|]([^|]*)[|]([^|]*)[|]([^|(;,]+).*/\3/' <<<"$list" | grep -Ei "[a-z-]+"
  # Full description language
  sed -E 's/^([a-z-]+)[|]([^|]*)[|]([^|]*)[|]([^|(;,]+).*/\4/' <<<"$list"
  # Supplemental UPS BS
  sed -E 's/^([A-Z-]+).*/\1/' <<<"$supplemental"
  # Seen in the wild
...
_dc::iso::639-2::regexp() {
  ...
  while read -r line; do
    printf "%s%s" "$sep" "$line"
    sep="|"
  done < <(_dc::iso::639-2::boot)
  printf ")"
}

dc::iso::639-2::regexp(){
  if [ ! "$_DC_PRIVATE_ISO639_2" ]; then
    _DC_PRIVATE_ISO639_2="$(_dc::iso::639-2::regexp)"
  fi
  printf "%s" "$_DC_PRIVATE_ISO639_2"
}
...
language::extract::lang(){
  local test="$1"
  # qualifier SDH | forced
  perl -C -Mutf8 -ne "printf if s/$_DC_PRIVATE_SUB_OPEN$(dc::iso::639-2::regexp)$_DC_PRIVATE_SUB_QUAL$_DC_PRIVATE_SUB_CLOSE/\2/i" <<< "$test" | tr '[:upper:]' '[:lower:]'
}

language::extract::qualifier(){
  local test="$1"
  perl -C -Mutf8 -ne "printf if s/$_DC_PRIVATE_SUB_OPEN$(dc::iso::639-2::regexp)$_DC_PRIVATE_SUB_QUAL$_DC_PRIVATE_SUB_CLOSE/\4/i" <<< "$test" | tr '[:upper:]' '[:lower:]'
}

language::extract::rest(){
  local test="$1"
  local x
  x="$(perl -C -Mutf8 -ne "printf if s/$_DC_PRIVATE_SUB_OPEN$(dc::iso::639-2::regexp)$_DC_PRIVATE_SUB_QUAL$_DC_PRIVATE_SUB_CLOSE/@-[1]/i" <<< "$test")"
  [ "$x" ] || x="${#test}"
  printf "%s" "${test:0:$x}"
}
...
Ita"
  ...
  sep="[\[()\] ._-]"
  perl -C -Mutf8 -pe "s/$sep$final.*//gi" <<<"$test"
}
